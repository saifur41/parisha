...
#########################
 $tutor_det=mysql_fetch_assoc(mysql_query("SELECT * FROM `gig_teachers` WHERE id=".$_GET['tid']));
 echo "<pre>";
priint_r($tutor_det);
exit;
 @extract($tutor_det);
  $user_state_arr=unserialize($tutor_det['signup_state']);
 $profile=mysql_fetch_assoc(mysql_query("SELECT * FROM `tutor_profiles` WHERE tutorid=".$_GET['tid']));
